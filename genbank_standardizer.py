...
If a genbank file does not contain locus_tags, it generates them and
re-saves the genbank file.
"""
...
for phage, attributes in phages.iteritems():
    ...
    #check whether locus_tags exist and, if not, generate them and save
    #again the genbank file
    ftoi_cnt=1 #feature of interest count
    ft_cnt=0 #feature count
    resave=0
    #iterate features
    for feat in gbkrecord.features:
        if feat.type in ['CDS', 'tRNA', 'rRNA']:
            if 'locus_tag' not in feat.qualifiers:
                resave=1
                gbkrecord.features[ft_cnt].qualifiers['locus_tag']=phage + \
                                            '_' + str(ftoi_cnt)
                ftoi_cnt=ftoi_cnt+1
        ft_cnt=ft_cnt+1
    #resave if needed
    if resave==1:
        print "---> Recoded: " + phage
        SeqIO.write(gbkrecord, gnome_path+'/genbank/'+attributes['Accession']
                           +'.gb', 'genbank')
